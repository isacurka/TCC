...
$sql = "SELECT * FROM catalogo_pecas";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<img src="' . $row['foto_peca'] . '">';
        echo '<h2>' . $row['nome_peca'] . '</h2>';
        echo '<p>' . $row['discri_peca'] . '</p>';
        echo '<p>Preço: R$ ' . number_format($row['valor_peca'], 2, ',', '.') . '</p>';
        echo '<a href="https://api.whatsapp.com/send?phone=5541997460677" class="button">Solicitar Serviço</a>';
        echo '</div>';
    }
} else {
