...
<body>
  ...
  <div class="form-group">
    <label for="service_name">Nome da Peça:</label>
    ...
    $query = "SELECT * FROM catalogo_pecas WHERE nome_peca LIKE '%$nome_peca%'";
    $result = $conn->query($query);

    if ($result->num_rows > 0) {
        // Loop through the results to display matching services
        while ($row = $result->fetch_assoc()) {
            echo "ID: " . $row['id_peca'] . "<br>";
            echo "Nome: " . $row['nome_peca'] . "<br>";
            echo "Discrição: " . $row['discri_peca'] . "<br>";
            echo "Valor: " . $row['valor_peca'] . "<br>";

            // Display the image associated with the service
            echo "<img src='" . $row['foto_peca'] . "' alt='Imagem do Serviço' width='300'><br><br>";

            echo "<a href='editarpeca.php?id=" . $row['id_peca'] . "'class='a' >Editar</a> ";
            echo "<a href='excluirpeca.php?id=" . $row['id_peca'] . "' class='a' onclick=\"return confirm('Tem certeza que deseja excluir?')\">Excluir</a><br><br>";
        }
    } else {
        echo "Nenhuma peça encontrada com o nome: " . $nome_peca;
    }
}
  ...
  <footer>
  <button class="cad-button" onclick="redirect()">Cadastrar Peças</button>
    <div class="container">
  ...
  <script>
     function redirect() {
        window.location.href = "cadpecas.php";
        }
  </script>
</body>
</html>